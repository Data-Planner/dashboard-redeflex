...
def create_metas_image(quality="fhd"):
    ...
    cell_detalhe = PIL.ImageFont.truetype(font_reg, int(12 * scale))
    footer_font = PIL.ImageFont.truetype(font_bold, int(10 * scale))
    logo_font = PIL.ImageFont.truetype(font_bold, int(20 * scale))

    data = [
        (1, "Atingir meta de gastos estabelecida para área no 2º Q26", "Realizado de despesas menor que o orçamento planejado para o quadrimestre."),
        (2, "Atingir a Receita de Ativação prevista para o Quadrimestre", "Garantir o acompanhamento das ativações mensalmente, atuando na redução da dispersão entre o que foi ativado e que realmente foi pago."),
        (3, "Cockpit de Gestão Comercial: Do Macro ao PDV", "Cockpit de Gestão Comercial: Do Macro ao PDV"),
        (4, "Análise de dados - Implantação de Inteligência Analítica Comercial", "Análise de dados - Implantação de Inteligência Analítica Comercial"),
        (5, "Criar o painel de POS PARADA visão consolidada (Move e DX) Visão Logistica", "Criar o painel de Gestão das POS visão consolidada (Move e DX) Visão Logistica. (Estoque / POS Parada / Modelos / depósitos / etc). Reduzir o % de POS parada em 30%. Reduzir o tempo de geração das informações do parque de máquinas e aumentar a confiabilidade do dado."),
    ]
    ...
    draw.text((width - int(240 * scale), logo_y + int(12 * scale)), "2º Quadrimestre 2026", font=date_font, fill=white)
    ...
    out = f"public/metas_2q_{quality}.png"
